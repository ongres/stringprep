...
public final class Stringprep {
  ...
  Stringprep(Profile profile, boolean storedStrings) {
    ...
    this.options = EnumSet.copyOf(Objects.requireNonNull(profile.profile()));
    if (options.contains(Option.CHECK_BIDI) && !options.contains(Option.FORBID_CHANGE_DISPLAY_AND_DEPRECATED)) {
      this.options.add(Option.FORBID_CHANGE_DISPLAY_AND_DEPRECATED);
    }
  }
  ...
  char[] prepare(final char[] string) {
    ...
    // normalization.
    if (options.contains(Option.NORMALIZE_KC)) {
    ...
    // If the mapping/normalization removes all chars, return the empty string
    if (value.length == 0) {
      return value;
    }
    ...
    for (int i = 0; i < value.length; i += Character.charCount(codePoint)) {
      ...
      // error.
      if (options.contains(Option.CHECK_BIDI)) {
        // 1) The characters in section 5.8 MUST be prohibited.
        // Option.FORBID_CHANGE_DISPLAY_AND_DEPRECATED
  ...
  private char[] map(char[] string) {
    try (SecureCharBuffer mapping = new SecureCharBuffer(string.length + 16)) {
      for (int codePoint, i = 0; i < string.length; i += Character.charCount(codePoint)) {
        codePoint = Character.codePointAt(string, i);

        if (options.contains(Option.MAP_TO_NOTHING) && Tables.mapToNothing(codePoint)) { // NOPMD
          ...
          // listed in Table B.1.
        } else if (options.contains(Option.NORMALIZE_KC) && options.contains(Option.CASE_FOLD_NFKC)) {
          // appendix B.2 is for profiles that also use Unicode
          // normalization form KC
          for (int cp : Tables.mapWithNfkc(codePoint)) {
            mapping.appendCodePoint(cp);
          }
        } else if (!options.contains(Option.NORMALIZE_KC) && options.contains(Option.CASE_FOLD_NO_NORMALIZATION)) {
        ...
        } else if (options.contains(Option.ADDITIONAL_MAPPING)) {
  ...
  private void prohibitedOutput(int codePoint) {
    ...
    // the profile
    checkProhibited(options.contains(Option.FORBID_ADDITIONAL_CHARACTERS),
        profile::prohibitedAdditionalCharacters, codePoint, "Prohibited code point");

    checkProhibited(options.contains(Option.FORBID_ASCII_SPACES),
        Tables::prohibitionAsciiSpace, codePoint, "Prohibited ASCII space");
    checkProhibited(options.contains(Option.FORBID_NON_ASCII_SPACES),
        Tables::prohibitionNonAsciiSpace, codePoint, "Prohibited non-ASCII space");
    checkProhibited(options.contains(Option.FORBID_ASCII_CONTROL),
        Tables::prohibitionAsciiControl, codePoint, "Prohibited ASCII control");
    checkProhibited(options.contains(Option.FORBID_NON_ASCII_CONTROL),
        Tables::prohibitionNonAsciiControl, codePoint, "Prohibited non-ASCII control");
    checkProhibited(options.contains(Option.FORBID_PRIVATE_USE),
        Tables::prohibitionPrivateUse, codePoint, "Prohibited private use character");
    checkProhibited(options.contains(Option.FORBID_NON_CHARACTER),
        Tables::prohibitionNonCharacterCodePoints, codePoint, "Prohibited non-character code point");
    checkProhibited(options.contains(Option.FORBID_SURROGATE),
        Tables::prohibitionSurrogateCodes, codePoint, "Prohibited surrogate code point");
    checkProhibited(options.contains(Option.FORBID_INAPPROPRIATE_FOR_PLAIN_TEXT),
        Tables::prohibitionInappropriatePlainText, codePoint, "Prohibited plain text code point");
    checkProhibited(options.contains(Option.FORBID_INAPPROPRIATE_FOR_CANON_REP),
        Tables::prohibitionInappropriateCanonicalRepresentation, codePoint, "Prohibited non-canonical code point");
    checkProhibited(options.contains(Option.FORBID_CHANGE_DISPLAY_AND_DEPRECATED),
        Tables::prohibitionChangeDisplayProperties, codePoint, "Prohibited control character");
    checkProhibited(options.contains(Option.FORBID_TAGGING),
        Tables::prohibitionTaggingCharacters, codePoint, "Prohibited tagging character");
